	...
	*/
	public function search($search, $filters, $rows = 0, $limit_from = 0, $sort = 'items.name', $order = 'asc', $count_only = FALSE)
	{
		...
		if ($count_only == TRUE) {
			...
			$this->db->select('MAX(items.warranty) AS warranty');
			...
			$this->db->select('MAX(items.wholesale_selling_price) AS wholesale_selling_price');
			...
			$this->db->select('MAX(people.last_name) AS supplier_name');
		...
		$this->db->join('people AS people', 'people.person_id = suppliers.person_id', 'left');
