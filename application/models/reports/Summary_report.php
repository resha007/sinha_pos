...
abstract class Summary_report extends Report
{
	...
	{
		$this->__common_where($inputs);
	}
	protected function _where_new(array $inputs)
	{
		if (empty($this->config->item('date_or_time_format'))) {
			$this->db->where('DATE(ospos_income_expense.date) BETWEEN ' . $this->db->escape($inputs['start_date']) . ' AND ' . $this->db->escape($inputs['end_date']));
		} else {
			$this->db->where('ospos_income_expense.date BETWEEN ' . $this->db->escape(rawurldecode($inputs['start_date'])) . ' AND ' . $this->db->escape(rawurldecode($inputs['end_date'])));
		}
	}
	...
	public function getSummaryIncomeExpense(array $inputs)
	{
		$total_income = $this->getIncome($inputs);
		$total_expense = $this->getExpense($inputs);
		...
		$data = array(
			...
			'profit'  => $total_income['income'] + $total_expense['expense']
		);

		return $data;
	}

	public function getIncome(array $inputs)
	{
		$income = 'SUM(ospos_income_expense.amount) as income';
		$this->db->select($income);

		//from
		$this->db->from('ospos_income_expense AS ospos_income_expense');
		...
		$this->db->where('ospos_income_expense.amount >', 0);
		$this->db->where('ospos_income_expense.status', 1);
		//$this->db->where('ospos_income_expense.table_name', 'sales');

		return $this->db->get()->row_array();
	}

	public function getExpense(array $inputs)
	{
		$income = 'SUM(ospos_income_expense.amount) as expense';
		$this->db->select($income);
