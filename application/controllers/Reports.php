...
//echo $submodule_id;
	...
	public function summary_income_expense($start_date, $end_date, $sale_type, $location_id = 'all')
	{
		$inputs = array('start_date' => $start_date, 'end_date' => $end_date, 'sale_type' => $sale_type, 'location_id' => $location_id);

		$this->load->model('reports/Summary_income_expense');
		$model = $this->Summary_income_expense;

		$report_data = $model->getData($inputs);
		$summary = $this->xss_clean($model->getSummaryIncomeExpense($inputs));
	...
	{
		$inputs = array('start_date' => $this->input->get('start_date'), 'end_date' => $this->input->get('end_date'), 'sale_type' => "", 'location_id' => "all");
		$this->load->model('reports/Summary_income_expense');
		$model = $this->Summary_income_expense;
		$summary = $this->xss_clean($model->getSummaryIncomeExpense($inputs));
	...
	//Summary Taxes report Imal
	public function summary_taxes_copy($start_date, $end_date, $sale_type, $location_id = 'all')
	{
		$inputs = array('start_date' => $start_date, 'end_date' => $end_date, 'sale_type' => $sale_type, 'location_id' => $location_id);

		$this->load->model('reports/Summary_taxes_copy');
		$model = $this->Summary_taxes;
		...
		$data = array(
			'title' => $this->lang->line('reports_taxes_summary_report_456'),
			'subtitle' => $this->_get_subtitle_report(array('start_date' => $start_date, 'end_date' => $end_date)),
			'headers' => $this->xss_clean($model->getDataColumns()),
			'data' => $tabular_data,
			'summary_data' => $summary
	...
	//sold items count for dashboard
	public function summary()
	{
		...
		$inputs = array('start_date' => $start_date, 'end_date' => $end_date, 'sale_type' => $sale_type, 'location_id' => 'all');
		...
		//$this->load->view('reports/tabular', $data);
		echo json_encode(array('summary' => $summary, 'data' => $tabular_data));
	}
}
