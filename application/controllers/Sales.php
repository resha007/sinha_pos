...
class Sales extends Secure_Controller
{
	...
	public function item_search()
	{
		...
		$suggestions = array_merge($suggestions, $this->Item->get_search_suggestions($this->sale_lib->get_sale_location(),$search, array('search_custom' => FALSE, 'is_deleted' => FALSE), TRUE));
	...
	public function change_mode()
	{
		...
		}else if ($mode == 'sale_wholesale') {
			$this->sale_lib->set_sale_type(SALE_TYPE_WHOLESALE);
		} else {
	...
	public function add()
	{
		...
		if ($mode == 'return' && $this->Sale->is_valid_receipt($item_id_or_number_or_item_kit_or_receipt)) {
			...
			if($this->session->userdata('sales_mode')=="wholesale"){
				if (!$this->sale_lib->add_item($item_id_or_number_or_item_kit_or_receipt, $quantity, $item_location, $discount, $discount_type, PRICE_MODE_WHOLESALE, NULL, NULL, $price)) {
					$data['error'] = $this->lang->line('sales_unable_to_add_item');
				} else {
	...
	public function edit_item($item_id)
	{
		...
		//$this->form_validation->set_rules('warranty', 'lang:sales_warranty', 'required|callback_numeric');
		...
		if ($this->form_validation->run() != FALSE) {
			$this->sale_lib->edit_item($item_id, $description, $serialnumber, $quantity, $discount, $discount_type, $price, $discounted_total, $warranty);
	...
	public function complete()
	{
		...
		$data['company_info'] = implode("\n", array(
			...
			'Facebook : ' . $this->config->item('website'),
			'Email : ' . $this->config->item('email'),
			'Reg No : ' . $this->config->item('fax')
		));

		$data['company_info_short'] = implode("\n", array(
			$this->config->item('address'),
			'Tel : ' . $this->config->item('phone')
		));
	...
	private function _reload($data = array())
	{
		...
		//imal
		//$data['change_warranty'] = $this->Employee->has_grant('sales_warranty', $this->Employee->get_logged_in_employee_info()->person_id);
	...
	public function shippingLabel($sale_id)
	{
		$data = $this->_load_sale_data($sale_id);
		$this->load->view('sales/shipping_label', $data);
		$this->sale_lib->clear_all();
	}
