...
class Sale_lib
{
	...
	public function get_register_mode_options()
	{
		...
		$register_modes['wholesale'] = $this->CI->lang->line('sales_wholesale');
	...
	public function add_item(&$item_id, $quantity = 1, $item_location, $discount = 0.0, $discount_type = 0, $price_mode = PRICE_MODE_STANDARD, $kit_price_option = NULL, $kit_print_option = NULL, $price_override = NULL, $description = NULL, $serialnumber = NULL, $sale_id = NULL, $include_deleted = FALSE, $print_option = NULL, $line = NULL, $warranty='new')
	{
		...
		//$warranty = $warranty; //imal
		if($warranty=='new'){
			$warranty = $item_info->warranty;
		}
		//$warranty_period = $item_info->warranty_period; //imal

		if ($price_mode == PRICE_MODE_STANDARD) {
			$price = $item_info->unit_price;
			$cost_price = $item_info->cost_price;
		} elseif ($price_mode == PRICE_MODE_WHOLESALE) {
			$price = $item_info->wholesale_selling_price;
			$cost_price = $item_info->cost_price;
		...
		if (!$itemalreadyinsale || $item_info->is_serialized) {
			$item = array(
				$insertkey => array(
					...
					'name' => $item_info->name,
					'warranty' => $warranty, //imal
					//'warranty_period'=>$warranty_period,//imal
	...
	public function edit_item($line, $description, $serialnumber, $quantity, $discount, $discount_type, $price, $discounted_total = NULL,$warranty)
	{
		$items = $this->get_cart();
		if (isset($items[$line])) {
			$line = &$items[$line];
	...
	public function return_entire_sale($receipt_sale_id)
	{
		...
		foreach ($this->CI->Sale->get_sale_items_ordered($sale_id)->result() as $row) {
			$this->add_item($row->item_id, -$row->quantity_purchased, $row->item_location, $row->discount, $row->discount_type, PRICE_MODE_STANDARD, NULL, NULL, $row->item_unit_price, $row->description, $row->serialnumber, NULL, TRUE,$row->warranty);
		}
	...
	public function copy_entire_sale($sale_id)
	{
		...
		foreach ($this->CI->Sale->get_sale_items_ordered($sale_id)->result() as $row) { 
			$this->add_item($row->item_id, $row->quantity_purchased, $row->item_location, $row->discount, $row->discount_type, PRICE_MODE_STANDARD, NULL, NULL, $row->item_unit_price, $row->description, $row->serialnumber, $sale_id, TRUE, $row->print_option,NULL,$row->warranty);
		}
